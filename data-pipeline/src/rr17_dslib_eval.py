...
OWN, JPF, DSL, SNAP, SCHEMA, REG, V5, OUT = map(Path, sys.argv[1:9])
...
R = {
    "config": dict(
        ...
        protocol="scenario B q0.7 per project; v4 params; variant B weights",
    )
}
...
# ---------- corpora ----------
own = pd.read_csv(OWN)
own = own[~own["project"].isin(EXCLUDE_OWN)].copy()
own["source"] = "own"
...
jpf = jpf[
    ~jpf["project"].isin(
        {
            p["contained"]
            for p in json.load(open(JPF.parent / "jpf_dedup_containment.json"))
        }
    )
].copy()
jpf["source"] = "jpf"
...
for d in (own, jpf):
    if "is_late_7" not in d:
        d["is_late_7"] = (
            (d["delay_days"] > 7).astype(float).where(d["delay_days"].notna())
        )
    for c in ("grew", "sector_group"):
        d[c] = np.nan if c == "grew" else "buildings" if d is own else "infrastructure"
cols = list(
    dict.fromkeys(
        [
            "project",
...
R["dslib"] = dict(
    projects=int(dsl_lab["project"].nunique()),
    labeled=int(len(dsl_lab)),
    holdout_projects=len(hold),
    holdout_labeled=int((dsl_lab["split"] == "holdout").sum()),
    by_group={
        g: dict(
            projects=int(x["project"].nunique()),
            labeled=int(len(x)),
            late_rate=round(float(x["is_late"].mean()), 3),
            grew_rate=round(float(x["grew"].mean()), 3),
        )
        for g, x in dsl_lab.groupby("sector_group")
    },
)
...
def ref_aucs(d, lbl):
    ...
    for f in ("rel_position", "planned_duration_days"):
        s = d[f].fillna(d[f].median())
        a = auc(d[lbl], s)
        out[f] = round(max(a, 1 - a), 4) if a is not None else None
    return out
...
def per_project(d, lbl, score):
    ...
    for _, g in d.assign(_s=score).groupby("project"):
        if len(g) < 30 or g[lbl].sum() < 5 or (len(g) - g[lbl].sum()) < 5:
            continue
...
versions = sorted(
    (p for p in REG.glob("v*") if p.name[1:].isdigit()), key=lambda p: int(p.name[1:])
)
v4 = joblib.load(versions[-1] / "classifier.joblib")
v5 = joblib.load(V5)
...
for lbl in ("is_late", "is_late_7", "grew"):
    m = dsl_lab[lbl].notna()
    d = dsl_lab[m]
    p4 = v4.predict_proba(d[NUM + CAT])[:, 1]
    p5 = v5.predict_proba(d[NUM + CAT])[:, 1]
    e = dict(
        n=int(len(d)),
        base_rate=round(float(d[lbl].mean()), 4),
        v4=dict(
            pooled=auc(d[lbl], p4),
            per_project=per_project(d, lbl, p4),
            mean_p=round(float(p4.mean()), 4),
            brier=brier(d[lbl], p4) if lbl == "is_late" else None,
        ),
        v5_candidate=dict(
            pooled=auc(d[lbl], p5),
            per_project=per_project(d, lbl, p5),
            mean_p=round(float(p5.mean()), 4),
        ),
        reference=ref_aucs(d, lbl),
        by_group={},
    )
    for grp, g in d.groupby("sector_group"):
        ...
        e["by_group"][grp] = dict(
            n=int(len(g)),
            base_rate=round(float(g[lbl].mean()), 3),
            v4=auc(g[lbl], p4[i]),
            v5_candidate=auc(g[lbl], p5[i]),
...
print(
    ...
    json.dumps(
        {
            lb: dict(
                v4=A["labels"][lb]["v4"]["pooled"],
                v5=A["labels"][lb]["v5_candidate"]["pooled"],
                ref=A["labels"][lb]["reference"],
            )
            for lb in A["labels"]
        },
        indent=1,
...
dsl_train = dsl_lab[dsl_lab["split"] == "train"]
dsl_hold = dsl_lab[dsl_lab["split"] == "holdout"]
pool = pd.concat([own[cols], jpf[cols], dsl_train[cols]], ignore_index=True)
pool = pool[pool["is_late"].notna()].copy()
pool["y_islate"] = pool["is_late"].astype(int)
pool["y_harmonised"] = np.where(
    pool["source"] == "dslib", pool["grew"], pool["is_late"]
)
pool = pool[pool["y_harmonised"].notna()]
pool["y_harmonised"] = pool["y_harmonised"].astype(int)
B_tr, B_te = temporal_split(pool)
R["part_B_corpus_v3"] = {
    "n_train": int(len(B_tr)),
    "n_test": int(len(B_te)),
    "projects_B": int(B_tr["project"].nunique()),
    "train_by_source": B_tr["source"].value_counts().to_dict(),
    "runs": {},
}
hold_X = dsl_hold
fitted = {}
for target in ("y_islate", "y_harmonised"):
    for name, est in models().items():
        ...
        pipe.fit(B_tr, B_tr[target], model__sample_weight=weights_B(B_tr))
        pt = pipe.predict_proba(B_te)[:, 1]
        ph = pipe.predict_proba(hold_X)[:, 1]
        run = {
            "pooled_test_auc": auc(B_te[target], pt),
            "slices": {
                s: auc(
                    B_te.loc[B_te["source"] == s, target],
                    pt[(B_te["source"] == s).to_numpy()],
                )
                for s in ("own", "jpf", "dslib")
            },
            "dslib_holdout": {
                lbl: dict(
                    all=auc(dsl_hold[lbl], ph),
                    per_project=per_project(dsl_hold, lbl, ph),
                    by_group={
                        g: auc(x[lbl], ph[(dsl_hold["sector_group"] == g).to_numpy()])
                        for g, x in dsl_hold.groupby("sector_group")
                    },
                )
                for lbl in ("is_late", "grew")
            },
        }
        R["part_B_corpus_v3"]["runs"][f"{name}__{target}"] = run
        fitted[(name, target)] = pipe
        print(
            name,
            target,
            json.dumps(run["slices"]),
            "holdout",
            json.dumps(
                {lb: run["dslib_holdout"][lb]["all"] for lb in run["dslib_holdout"]}
            ),
        )
R["part_B_corpus_v3"]["references"] = dict(
    rr16_rf_own_slice=0.758,
    v4_selection_fit=0.7505,
    v5_transfer_holdout_is_late=auc(
        dsl_hold["is_late"], v5.predict_proba(dsl_hold[NUM + CAT])[:, 1]
    ),
    v5_transfer_holdout_grew=auc(
        dsl_hold["grew"], v5.predict_proba(dsl_hold[NUM + CAT])[:, 1]
    ),
    v4_transfer_holdout_is_late=auc(
        dsl_hold["is_late"], v4.predict_proba(dsl_hold[NUM + CAT])[:, 1]
    ),
    v4_transfer_holdout_grew=auc(
        dsl_hold["grew"], v4.predict_proba(dsl_hold[NUM + CAT])[:, 1]
    ),
)
...
own_te = B_te[B_te["source"] == "own"]
C = {
    "n_fit": int(len(B_fit)),
    "n_cal": int(len(B_cal)),
    "own_test_n": int(len(own_te)),
    "own_brier_dummy": dummy_brier(own_te["y_islate"]),
}


def calib(sub):
    c = CalibratedClassifierCV(FrozenEstimator(rf), method="sigmoid")
    c.fit(sub, sub["y_islate"])
    return c


cal_pooled = calib(B_cal)
cal_own = calib(B_cal[B_cal["source"] == "own"])
bld_cal = B_cal[
    (B_cal["source"] == "own")
    | ((B_cal["source"] == "dslib") & (B_cal["sector_group"] == "buildings"))
]
cal_bld = calib(bld_cal)
C["own_slice"] = {
    "auc_uncalibrated": auc(own_te["y_islate"], rf.predict_proba(own_te)[:, 1]),
    "brier_pooled_calibrator": brier(
        own_te["y_islate"], cal_pooled.predict_proba(own_te)[:, 1]
    ),
    "brier_own_calibrator": brier(
        own_te["y_islate"], cal_own.predict_proba(own_te)[:, 1]
    ),
    "brier_buildings_calibrator_own+dslib": brier(
        own_te["y_islate"], cal_bld.predict_proba(own_te)[:, 1]
    ),
    "n_cal_own": int((B_cal["source"] == "own").sum()),
    "n_cal_buildings": int(len(bld_cal)),
}
hb = dsl_hold[dsl_hold["sector_group"] == "buildings"]
C["dslib_holdout_buildings"] = {
    "n": int(len(hb)),
    "brier_dummy": dummy_brier(hb["is_late"]),
...
R["part_C_calibration"] = C
print("Part C:", json.dumps(C["own_slice"]))
joblib.dump(cal_bld, OUT / "v6_candidate_rf_buildings_calibrator.joblib", compress=3)

# ---------- Part D: as-of on real snapshots ----------
snap = pd.read_csv(SNAP, parse_dates=["status_date", "actual_start"])
snap["project"] = "dslib::" + snap["project"].astype(str)
meta_cols = dsl.set_index(["project", "task_id"])[
    [
        "rel_position",
        "proj_span_days",
        "is_late",
...
bstart = pd.to_datetime(
    dsl.set_index(["project", "task_id"])["baseline_start"]
)  # frozen baseline start per task
...
for (pid, tp), g in snap.groupby(["project", "tp"]):
    ...
    started = g[g["status"] != "not_started"]
    ns = g[g["status"] == "not_started"]
    idx_ns = list(zip(ns["project"], ns["task_id"]))
    lab_ns = meta_cols.reindex(idx_ns)
    lab_ns = lab_ns[lab_ns["is_late"].notna()]
    if len(fin) < 5 or len(lab_ns) < 5:
        continue
    sd = g["status_date"].iloc[0]
    st_idx = list(zip(started["project"], started["task_id"]))
    slip = (
        (started["actual_start"].to_numpy() - bstart.reindex(st_idx).to_numpy())
        .astype("timedelta64[D]")
        .astype(float)
    )
    fin_meta = meta_cols.reindex(list(zip(fin["project"], fin["task_id"])))
    p0 = bstart.loc[pid].min()
    span = meta_cols.loc[pid, "proj_span_days"].iloc[0]
    feats = dict(
        elapsed_share=float((sd - p0).days / span) if span else np.nan,
        share_started=len(started) / len(g),
        share_finished=len(fin) / len(g),
        median_start_slip_so_far=float(np.nanmedian(slip)) if len(slip) else 0.0,
        share_finished_late_so_far=float(fin_meta["is_late"].mean())
        if fin_meta["is_late"].notna().any()
        else np.nan,
        mean_grew_so_far=float(fin_meta["grew"].mean())
        if fin_meta["grew"].notna().any()
        else np.nan,
    )
...
inst_df = pd.DataFrame(inst).merge(
    dsl[["project", "task_id", *NUM]], on=["project", "task_id"], how="left"
)
...
ASOF = [
    ...
    "share_started",
    "share_finished",
    "median_start_slip_so_far",
    "share_finished_late_so_far",
    "mean_grew_so_far",
]
D = {
    "n_instances": int(len(inst_df)),
    "n_snapshots": int(inst_df.groupby(["project", "tp"]).ngroups),
    "n_projects": int(inst_df["project"].nunique()),
    "base_rate_is_late": round(float(inst_df["is_late"].mean()), 4),
    "base_rate_grew": round(float(inst_df["grew"].mean()), 4),
    "results": {},
}
...
for lbl in ("is_late", "grew"):
    ...
    for name, extra in (("plan_time_only", []), ("plan_time_plus_asof", ASOF)):
        oof = np.zeros(len(inst_df))
        for tr, te in gkf.split(inst_df, inst_df[lbl], groups=inst_df["project"]):
            pipe = Pipeline(
                [
                    ("prep", prep_asof(extra)),
                    (
                        "model",
                        RandomForestClassifier(
            ...
            pipe.fit(inst_df.iloc[tr], inst_df.iloc[tr][lbl])
            oof[te] = pipe.predict_proba(inst_df.iloc[te])[:, 1]
        res[name] = dict(
            auc=auc(inst_df[lbl], oof), per_project=per_project(inst_df, lbl, oof)
        )
    a = auc(inst_df[lbl], inst_df["median_start_slip_so_far"].fillna(0))
    res["naive_rule_median_slip"] = round(max(a, 1 - a), 4)
    a = auc(inst_df[lbl], inst_df["share_finished_late_so_far"].fillna(0))
    res["naive_rule_share_late"] = round(max(a, 1 - a), 4)
    res["gain_asof"] = round(
        res["plan_time_plus_asof"]["auc"] - res["plan_time_only"]["auc"], 4
    )
    D["results"][lbl] = res
