...
vdir = versions[-1]
clf = joblib.load(vdir / "classifier.joblib")
...
res = {
    "served_version": vdir.name,
    "excluded_contained_projects": excluded,
    "n_projects": int(lab["project"].nunique()),
...
res["brier_dummy_jpf_rate"] = round(
    float(((lab["is_late"].mean() - lab["is_late"]) ** 2).mean()), 4
)
...
for f in ("planned_duration_days", "upstream_cnt", "total_float_hr"):
    s = lab[f].fillna(lab[f].median())
    a = auc(lab["is_late"], s)
    res[f"ref_auc_{f}"] = (
        round(max(a, 1 - a), 4) if a is not None else None
    )  # direction-agnostic
# H2 — per-project dispersion
per = []
for pid, g in lab.groupby("project"):
    if len(g) < 30 or g["is_late"].sum() < 5 or (len(g) - g["is_late"].sum()) < 5:
        continue
    per.append(
        dict(
            project=pid,
            type=g["project_type"].iloc[0],
            n=int(len(g)),
            late_rate=round(float(g["is_late"].mean()), 3),
            auc=round(auc(g["is_late"], g["p"]), 4),
        )
    )
pa = np.array([x["auc"] for x in per])
res["per_project"] = dict(
    n_projects=len(per),
    median_auc=round(float(np.median(pa)), 4),
    q25=round(float(np.percentile(pa, 25)), 4),
    q75=round(float(np.percentile(pa, 75)), 4),
    min=round(float(pa.min()), 4),
...
show = {
    ...
    if k not in ("per_project_table", "excluded_contained_projects")
}
print(json.dumps(show, indent=1))

# figure — same conventions as scenario_a_stability.png
import matplotlib  # noqa: E402 — headless backend must be chosen before pyplot is imported
...
ax1.annotate(
    f"median {np.median(pa):.2f}",
    xy=(np.median(pa), ax1.get_ylim()[1] * 0.9),
    xytext=(6, 0),
    textcoords="offset points",
    fontsize=9,
    color="#1F3864",
)
ax1.set_xlabel("ROC-AUC per JPF project (v4, untouched)")
ax1.set_ylabel("Projects")
ax1.set_title(f"RR-15 — per-project transfer AUC ({len(per)} projects)")
...
vals = [
    res["pooled_auc_is_late"],
    res["pooled_auc_is_late_7"],
    res["pooled_auc_grew"],
    meta["served_metrics_B_temporal"]["roc_auc"],
    0.68,
]
...
ax2.set_title("Where the JPF transfer sits")
ax2.grid(alpha=0.25, lw=0.5, axis="y")
plt.tight_layout()
(OUT / "figures").mkdir(exist_ok=True)
plt.savefig(OUT / "figures" / "rr15_jpf.png")
print("figure saved")
