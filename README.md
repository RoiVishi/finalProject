...
| `ai-service/` | שירות חיזוי עיכובים (REST) | FastAPI | פעיל (v0.2) |
...
## תוצאות המודל — מצב עדכני (16.8.2026, registry v4)
...
**תרחיש A — פרויקט חדש לגמרי** (GroupShuffleSplit לפי פרויקט): האלוף כמעט אקראי (AUC ‏0.57), והתוצאות תנודתיות מאוד בין מודלים על פיצול יחיד — ניסוי פיצולים חוזרים מתוכנן. המסקנה המוצרית: חיזוי מבוסס-היסטוריה בלבד.
...
- **ניסויים מרכזיים:** עקומת למידה RR-11 (חיזוי שמיש מ-40% היסטוריה); אבלציית float ‏RR-12 (המודל בלי float מאבד 0.0015 AUC בלבד ⇒ אין צורך ב-CPM עבור החיזוי).
...
- [x] קוד ב-Git, בדיקות יחידה (ai-service, 15 בדיקות), CI (GitHub Actions)
- [x] API חיצוני (שירות AI כ-REST), דאטהסט אמיתי + pipeline מתועד ומשוחזר
- [x] Jira מקושר לדרישות; מסמך דרישות v1.5 (טרם הוגש למנחה)
