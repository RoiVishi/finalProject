...
SRC = Path(sys.argv[1]); REG = Path(sys.argv[2]); OUT = Path(sys.argv[3] if len(sys.argv) > 3 else "outputs")
...
def graph_stats(ids, edges):
    ...
    def reach(start, adj, cap=BFS_CAP):
        ...
        return len(seen) - 1
    return {t: dict(n_pred=len(pred[t]), n_succ=len(succ[t]),
                    upstream_cnt=reach(t, pred), downstream_cnt=reach(t, succ)) for t in ids}

def open_json(p: Path):
    if p.suffix == ".zip":
        z = zipfile.ZipFile(p); name = [n for n in z.namelist() if n.endswith(".json") and not n.startswith("__")][0]
        return z.open(name)
    return open(p, "rb")

rows = []; task_id_projects = defaultdict(set)
with open_json(SRC) as f:
    for pid, proj in ijson.kvitems(f, "Projects"):
        ...
        for aid, a in acts.items():
            ...
            if isinstance(rels, dict):
                for r in rels.values():
                    ...
                    if p_: edges.append((str(p_), str(aid)))
        ...
        starts = [d for d in starts if d]; fins = [d for d in fins if d]
        ...
        for aid, a in acts.items():
            ...
            ps, pf = pdate(a.get("Planned_Start_Date")), pdate(a.get("Planned_Finish_Date"))
            as_, af = pdate(a.get("Actual_Start_Date")), pdate(a.get("Actual_Finish_Date"))
            gs = g[aid]
            delay = (af.date() - pf.date()).days if (af and pf) else np.nan
            ode, aod = fnum(a.get("Original_Duration_Estimate(ODE)")), fnum(a.get("At_Completion_Duration"))
            rows.append(dict(
                project=pid, project_type=proj.get("Project_Type"), region=proj.get("UK_Region"),
                task_id=aid,
                ...
                rel_position=(((ps - p0).days / span) if ps and p0 and span == span else np.nan),
                proj_n_tasks=len(acts), proj_span_days=span,
                **gs, task_type=None,
                started_early_days=((ps.date() - as_.date()).days if ps and as_ else np.nan),
                delay_days=delay,
                is_late=(float(delay > 0) if delay == delay else np.nan),
                is_late_7=(float(delay > 7) if delay == delay else np.nan),
                grew=(float(aod > ode) if (ode == ode and aod == aod and (af is not None)) else np.nan),
            ))
...
summary = dict(projects=int(df["project"].nunique()), activities=int(len(df)), labeled=int(len(lab)),
               late_rate=round(float(lab["is_late"].mean()), 4), late7_rate=round(float(lab["is_late_7"].mean()), 4),
               grew_rate=round(float(lab["grew"].mean()), 4), task_ids_shared_across_projects=dups)
