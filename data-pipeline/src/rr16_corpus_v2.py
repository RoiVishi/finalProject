...
OWN, JPF, SCHEMA, OUT = Path(sys.argv[1]), Path(sys.argv[2]), Path(sys.argv[3]), Path(sys.argv[4])
...
# ---------- corpus ----------
own = pd.read_csv(OWN); own = own[~own["project"].isin(EXCLUDE_OWN)]; own["source"] = "own"
own["project"] = "own::" + own["project"].astype(str)
jpf = pd.read_csv(JPF, dtype={"project": str, "task_id": str})
jpf_excl = {p["contained"] for p in json.load(open(JPF.parent / "jpf_dedup_containment.json"))}
jpf = jpf[~jpf["project"].isin(jpf_excl)]; jpf["source"] = "jpf"; jpf["project"] = "jpf::" + jpf["project"]
jpf["task_type"] = np.nan
if "is_late_7" not in own: own["is_late_7"] = (own["delay_days"] > 7).astype(float).where(own["delay_days"].notna())
cols = ["project", "source", *NUM, *CAT, "rel_position", "delay_days", "is_late", "is_late_7"]
cols = list(dict.fromkeys(cols))
pool = pd.concat([own[cols], jpf[cols]], ignore_index=True)
lab = pool[pool["is_late"].notna()].copy(); lab["is_late"] = lab["is_late"].astype(int)
...
def temporal_split(df):
    ...
    for name, g in df.groupby("project"):
        ...
        if len(a) >= MIN_PER_SIDE and len(b) >= MIN_PER_SIDE: tr.append(a); te.append(b); projects.append(name)
    return pd.concat(tr), pd.concat(te), projects

B_tr, B_te, B_projects = temporal_split(lab)
print(f"pooled corpus: {len(lab)} labeled | scenario B: {len(B_projects)} projects, "
...
def weights(df, variant):
    if variant == "A": return None
    if variant == "B":
        w = 1.0 / df["source"].map(df["source"].value_counts()); return (w / w.mean()).to_numpy()
    if variant == "C":
        n = df["project"].map(df["project"].value_counts()); w = np.sqrt(n) / n; return (w / w.mean()).to_numpy()

def metrics(y, pred, proba):
    return dict(auc=round(float(roc_auc_score(y, proba)), 4) if len(np.unique(y)) > 1 else None,
...
results = {"config": dict(seed=SEED, protocol="scenario B per-project q0.7; v4 params, no re-tuning",
                          n_projects=len(B_projects), n_train=len(B_tr), n_test=len(B_te),
                          n_own_projects=sum(p.startswith("own::") for p in B_projects),
                          own_excluded=sorted(EXCLUDE_OWN), jpf_excluded_contained=sorted(jpf_excl),
                          base_rate_train=round(float(B_tr["is_late"].mean()), 4),
                          base_rate_test=round(float(B_te["is_late"].mean()), 4),
                          base_rate_test_by_source=B_te.groupby("source")["is_late"].mean().round(4).to_dict()),
           "variants": {}}
own_te, jpf_te = B_te[B_te["source"] == "own"], B_te[B_te["source"] == "jpf"]
fitted = {}
for variant in ("A", "B", "C"):
    w = weights(B_tr, variant); results["variants"][variant] = {}
    for name, est in zoo().items():
        pipe = Pipeline([("prep", prep()), ("model", est)])
        fit_kw = {} if (w is None or name == "dummy_majority") else {"model__sample_weight": w}
        pipe.fit(B_tr, B_tr["is_late"], **fit_kw)
        proba = pipe.predict_proba(B_te)[:, 1]; pred = (proba >= 0.5).astype(int)
        r = {"pooled_test": metrics(B_te["is_late"], pred, proba),
             "own_slice": metrics(own_te["is_late"], pred[(B_te["source"] == "own").to_numpy()], proba[(B_te["source"] == "own").to_numpy()]),
             "jpf_slice": metrics(jpf_te["is_late"], pred[(B_te["source"] == "jpf").to_numpy()], proba[(B_te["source"] == "jpf").to_numpy()])}
        if name == "xgboost":
            p7 = pipe.predict_proba(B_te)[:, 1]
            r["pooled_test_label_gt7d_auc"] = round(float(roc_auc_score(B_te["is_late_7"], p7)), 4)
        results["variants"][variant][name] = r; fitted[(variant, name)] = pipe
        print(f"[{variant}] {name:20s} pooled AUC={r['pooled_test']['auc']}  own-slice AUC={r['own_slice']['auc']}  jpf-slice AUC={r['jpf_slice']['auc']}")

# ---------- H2 decision + calibration on the best variant by OWN-slice AUC (product domain) ----------
best_variant = max(("A", "B", "C"), key=lambda v: results["variants"][v]["xgboost"]["own_slice"]["auc"] or 0)
results["decision"] = {"best_variant_by_own_slice": best_variant,
                       "own_slice_auc_v4_reference_selection_fit": 0.7505,
                       "own_slice_auc_v2": results["variants"][best_variant]["xgboost"]["own_slice"]["auc"]}
fit_parts, cal_parts = [], []
...
champ = Pipeline([("prep", prep()), ("model", XGBClassifier(random_state=SEED, n_jobs=-1, eval_metric="logloss", tree_method="hist", **XGB_PARAMS))])
champ.fit(B_fit, B_fit["is_late"], **({} if wf is None else {"model__sample_weight": wf}))
cal = CalibratedClassifierCV(FrozenEstimator(champ), method="sigmoid"); cal.fit(B_cal, B_cal["is_late"])
pc = cal.predict_proba(B_te)[:, 1]; pu = champ.predict_proba(B_te)[:, 1]
results["served_candidate"] = {
    "variant": best_variant, "n_fit": len(B_fit), "n_cal": len(B_cal),
    "pooled": {"auc": round(float(roc_auc_score(B_te["is_late"], pc)), 4), "brier_calibrated": round(float(brier_score_loss(B_te["is_late"], pc)), 4),
               "brier_uncalibrated": round(float(brier_score_loss(B_te["is_late"], pu)), 4)},
    "by_source": {s: {"auc": round(float(roc_auc_score(g["is_late"], pc[(B_te["source"] == s).to_numpy()])), 4),
                      "brier_calibrated": round(float(brier_score_loss(g["is_late"], pc[(B_te["source"] == s).to_numpy()])), 4),
                      "brier_dummy_source_rate": round(float(((g["is_late"].mean() - g["is_late"]) ** 2).mean()), 4),
                      "base_rate": round(float(g["is_late"].mean()), 4)} for s, g in B_te.groupby("source")}}
cand = OUT / "registry_candidate" / "v5-candidate"; cand.mkdir(parents=True, exist_ok=True)
...
for proj in B_projects:
    ...
    if g["is_late"].sum() < 5 or (len(g) - g["is_late"].sum()) < 5: continue
    tr = lab[lab["project"] != proj]; wtr = weights(tr, best_variant)
    pipe = Pipeline([("prep", prep()), ("model", XGBClassifier(random_state=SEED, n_jobs=-1, eval_metric="logloss", tree_method="hist", **XGB_PARAMS))])
    pipe.fit(tr, tr["is_late"], **({} if wtr is None else {"model__sample_weight": wtr}))
    lopo.append({"project": proj, "source": g["source"].iloc[0], "n": int(len(g)),
                 "auc": round(float(roc_auc_score(g["is_late"], pipe.predict_proba(g)[:, 1])), 4)})
la = np.array([x["auc"] for x in lopo])
results["lopo"] = {"n_projects": len(lopo), "median": round(float(np.median(la)), 4),
                   "q25": round(float(np.percentile(la, 25)), 4), "q75": round(float(np.percentile(la, 75)), 4),
                   "share_above_chance": round(float((la > 0.5).mean()), 3),
                   "own_projects_median": round(float(np.median([x["auc"] for x in lopo if x["source"] == "own"])), 4) if any(x["source"] == "own" for x in lopo) else None,
...
print(json.dumps({k: v for k, v in results.items() if k != "lopo"} | {"lopo": {k: v for k, v in results["lopo"].items() if k != "table"}}, indent=1, default=str))
