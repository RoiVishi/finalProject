...
def main() -> int:
    ...
    def heuristic_score(df):
        f = df["total_float_hr"]
        worst = f.min() - 1 if f.notna().any() else -1
        return -f.fillna(worst)
    s_tr, s_te = heuristic_score(tr), heuristic_score(te)
    ...
    for x in cands:
        pred = (tr["total_float_hr"].fillna(-1) < x).astype(int)
        f1 = f1_score(y_tr, pred, zero_division=0)
        if f1 > best_f1:
            best_f1, best_x = f1, float(x)
    pred_te = (te["total_float_hr"].fillna(-1) < best_x).astype(int)
    results["cpm_heuristic"] = {
        ...
        "rule": f"late if total_float_hr < {best_x:.0f}h (threshold chosen on train)",
