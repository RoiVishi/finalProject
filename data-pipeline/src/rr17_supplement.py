...
own["is_late"] = binar(own["is_late"])
jpf = pd.read_csv(JPF, dtype={"project": str})
jpf = jpf[jpf["is_late"].notna()].copy()
jpf["is_late"] = jpf["is_late"].astype(int)
dsl = pd.read_csv(DSL)
dsl = dsl[dsl["is_late"].notna()].copy()
dsl["is_late"] = dsl["is_late"].astype(int)
dsl["grew"] = dsl["grew"].astype(int)
jg = jpf[jpf["grew"].notna()].copy()
jg["grew"] = jg["grew"].astype(int)
...
def direction(x, lbl):
    ...
    for _, g in x.groupby("project"):
        if len(g) >= 30 and min(g[lbl].sum(), len(g) - g[lbl].sum()) >= 5:
            r.append(roc_auc_score(g[lbl], g["rel_position"].fillna(0)))
            rd.append(roc_auc_score(g[lbl], g["planned_duration_days"].fillna(0)))
    r, rd = np.array(r), np.array(rd)
    return dict(
        projects=len(r),
        auc_position_median=round(float(np.median(r)), 3),
        share_position_below_chance=round(float((r < 0.5).mean()), 2),
...
def terciles(x, lbls):
    ...
    return {
        lb: x.groupby("pos_bin", observed=True)[lb].mean().round(3).to_dict()
        for lb in lbls
    }
...
R["post_hoc_label_construct"] = {
    ...
    "own_is_late_updated_plan": direction(own, "is_late"),
    "jpf_is_late": direction(jpf, "is_late"),
    "jpf_grew": direction(jg, "grew"),
    "dslib_is_late_frozen_baseline": direction(dsl, "is_late"),
    "dslib_grew": direction(dsl, "grew"),
    "late_rate_by_position_tercile": {
        "own": terciles(own, ["is_late"]),
        "dslib": terciles(dsl, ["is_late", "grew"]),
    },
}
...
noproj = [c for c in NUM if c not in ("proj_n_tasks", "proj_span_days")]
variants = {
    "plan_time_only_without_project_size": noproj,
    "asof_only_rf": ASOF,
    "asof_only_logreg": ASOF,
    "plan_time_without_project_size_plus_asof": noproj + ASOF,
}
D = {
    "note": "POST-HOC variants of Part D (GroupKFold by project, 5 folds). as-of features are project-level, hence constant within a (project, TP) — they can move pooled AUC, not within-project ranking.",
    "results": {},
}
for lbl in ("is_late", "grew"):
    ...
    for name, cols in variants.items():
        ...
        for tr, te in gkf.split(inst_df, inst_df[lbl], groups=inst_df["project"]):
            ...
            pipe.fit(inst_df.iloc[tr][cols], inst_df.iloc[tr][lbl])
            oof[te] = pipe.predict_proba(inst_df.iloc[te][cols])[:, 1]
        D["results"][lbl][name] = dict(
            auc=round(float(roc_auc_score(inst_df[lbl], oof)), 4),
            per_project=per_project(inst_df, lbl, oof),
        )
R["post_hoc_part_D_variants"] = D
json.dump(R, open(OUT / "rr17_dslib.json", "w"), indent=1, default=str)
print(
    json.dumps(
        {"label_construct": R["post_hoc_label_construct"], "partD_variants": D},
        indent=1,
        default=str,
    )
)
