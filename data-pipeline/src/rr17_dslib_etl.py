...
import openpyxl  # noqa: E402 — openpyxl warns on every DSLIB workbook extension; silence first

ROOT = Path(sys.argv[1])
OUT = Path(sys.argv[2] if len(sys.argv) > 2 else "outputs")
...
for r in list(
    openpyxl.load_workbook(
        ROOT / "DSLIB_Analysis_Sheet.xlsx", read_only=True, data_only=True
    )["DSLIB"].iter_rows(values_only=True)
)[3:]:
...
def sector_group(s):
    s = (s or "").lower()
    if not s.startswith("construction"):
        return None
    return (
        "buildings" if "building" in s else ("civil" if "civil" in s else "industrial")
    )
...
def add_wd(start, wdays, weekend=(5, 6)):
    if wdays is None or start is None:
        return None
    d = start
    n = int(round(wdays))
    if n <= 0:
...
def cpm_floats(ids, dur, preds):
    ...
    if len(order) != len(ids):
        return {t: (np.nan, np.nan) for t in ids}
            ...
            slack.append(
                {
                    "FS": ES[j] - (EF[i] + lag),
                    "SS": ES[j] - (ES[i] + lag),
                    "FF": (ES[j] + d_j) - (EF[i] + lag),
                    "SF": (ES[j] + d_j) - (ES[i] + lag),
                }[t]
            )
...
for f in sorted((ROOT / "Excel").glob("*.xlsx")):
    ...
    leaf = {
        k: bool(v) and not any(o != v and o.startswith(v + ".") for o in allw)
        for k, v in wbs.items()
    }
    if not allw:
        leaf = {
            k: True for k in wbs
        }  # 8 newer workbooks export no WBS column → every non-summary row is a leaf
    ...
    base = {
        r[0]: dict(
            ...
            b_end=todt(r[6]),
            b_dur=span_wd(r[7]),
            preds=parse_preds(r[3]),
        )
        for r in recs
    }
    for b in (
        base.values()
    ):  # same workbooks omit Baseline End → derive it from start + working-day duration
        if b["b_end"] is None and b["b_start"] is not None and b["b_dur"] is not None:
            b["b_end"] = add_wd(b["b_start"], b["b_dur"])
            derived_end[0] += 1
    preds = {
        k: [(i, t, lag) for i, t, lag in v["preds"] if i in ids]
        for k, v in base.items()
        if k in ids
    }
    dur = {k: (base[k]["b_dur"] or 0.0) for k in ids}
    fl = cpm_floats(ids, dur, preds)
    if any(np.isnan(v[0]) for v in fl.values()):
        cycles += 1
    edges = [(i, j) for j, lst in preds.items() for i, _, _ in lst]
    g = graph_stats(ids, edges)
    starts = [base[k]["b_start"] for k in ids if base[k]["b_start"]]
    ends = [base[k]["b_end"] for k in ids if base[k]["b_end"]]
    ...
    tps = [
        ...
        if s.startswith("Project Control - TP") or re.fullmatch(r"TP\d+", s)
    ]
    last = {}
    for k, sn in enumerate(tps, 1):
        tr = list(w[sn].iter_rows(values_only=True))
        sd = todt(tr[0][2])
        for r in tr[4:]:
            if not r or r[0] not in ids:
                continue
            a_start, a_dur, pc, st = (
                todt(r[11]),
                span_wd(r[12]),
                r[20],
                str(r[21] or "").strip().lower(),
            )
            done = (pc == 1) or (st == "finished")
            status = (
                "finished"
                if done
                else ("in_progress" if a_start is not None else "not_started")
            )
            snaps.append(
                dict(
                    project=code,
            ...
            if k == len(tps):
                last[r[0]] = (a_start, a_dur, done)
    for k in ids:
        b = base[k]
        a_start, a_dur, done = last.get(k, (None, None, False))
        labeled = (
            done
            and a_start is not None
            and a_dur is not None
            and b["b_start"]
            and b["b_end"]
        )
        ...
        rows.append(
            dict(
                ...
                sector=m.get("sector"),
                sector_group=grp,
                auth_tracking=m.get("auth_tracking"),
                task_id=k,
                task_name=b["name"],
                task_type=None,
                baseline_start=b["b_start"],
                baseline_end=b["b_end"],
                planned_duration_days=(
                    (b["b_end"] - b["b_start"]).days
                    if b["b_start"] and b["b_end"]
                    else np.nan
                ),
                planned_duration_wd=b["b_dur"],
                total_float_hr=fl[k][0] * HOURS_PER_DAY,
                free_float_hr=fl[k][1] * HOURS_PER_DAY,
                rel_position=(
                    ((b["b_start"] - p0).days / span)
                    if b["b_start"] and p0 and span == span
                    else np.nan
                ),
                proj_n_tasks=len(ids),
                proj_span_days=span,
                n_tp=len(tps),
                **g[k],
                actual_start=a_start if labeled else None,
                actual_duration_wd=a_dur if labeled else np.nan,
                actual_end=a_end,
                delay_days=delay,
                start_delay_days=sdel,
                is_late=(float(delay > 0) if labeled else np.nan),
                is_late_7=(float(delay > 7) if labeled else np.nan),
    ...
    proj_summ.append(
        dict(
            ...
            labeled=sum(
                1
                for k in ids
                if last.get(k, (None, None, False))[2] and last[k][0] is not None
            ),
        )
    )
...
df.to_csv(OUT / "dslib_labeled_tasks.csv", index=False)
sn.to_csv(OUT / "dslib_snapshots.csv", index=False)
lab = df[df["is_late"].notna()]
summary = dict(
    projects_construction=int(df["project"].nunique()),
    leaf_activities=int(len(df)),
    labeled=int(len(lab)),
    projects_with_labels=int(lab["project"].nunique()),
    by_group={
        g: dict(
            projects=int(x["project"].nunique()),
            activities=int(len(x)),
            labeled=int(x["is_late"].notna().sum()),
        )
        for g, x in df.groupby("sector_group")
    },
    late_rate=round(float(lab["is_late"].mean()), 4),
    late7_rate=round(float(lab["is_late_7"].mean()), 4),
    start_late_rate=round(float(lab["start_late"].mean()), 4),
    grew_rate=round(float(lab["grew"].mean()), 4),
    own_delay_rate=round(float(lab["own_delay"].mean()), 4),
    late_given_start_late=round(
        float(lab.loc[lab["start_late"] == 1, "is_late"].mean()), 4
    ),
    late_given_start_on_time=round(
        float(lab.loc[lab["start_late"] == 0, "is_late"].mean()), 4
    ),
    median_delay_days=float(lab["delay_days"].median()),
    median_start_delay_days=float(lab["start_delay_days"].median()),
    float_nan_share=round(float(df["total_float_hr"].isna().mean()), 4),
    projects_with_network_cycle=cycles,
    baseline_end_derived_cells=derived_end[0],
    snapshots_rows=int(len(sn)),
    tracking_periods=int(sn.groupby(["project", "tp"]).ngroups) if len(sn) else 0,
