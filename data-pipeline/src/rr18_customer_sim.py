...
DSL, INST, OWN, SCHEMA, OUT = map(Path, sys.argv[1:6]); OUT.mkdir(parents=True, exist_ok=True)
...
EXCLUDE_OWN = {"MERGE PROJECTS", "Enppi", "schedule of Medor", "HHI", "Ma'aden", "el ezz warehouse", "Baseline 15-11-2010", "Beyti plant baseline -b(fm-baseline)"}
...
def per_project(d, lbl, score, min_n=30):
    ...
    for _, g in d.assign(_s=score).groupby("project"):
        if len(g) < min_n or g[lbl].sum() < 5 or (len(g) - g[lbl].sum()) < 5: continue
...
def models():
    return {"dummy": DummyClassifier(strategy="prior"),
            "logistic_regression": LogisticRegression(max_iter=2000, class_weight="balanced", C=10.0),
            "random_forest": RandomForestClassifier(class_weight="balanced", random_state=SEED, n_jobs=-1, **RF_PARAMS),
            "xgboost": XGBClassifier(random_state=SEED, n_jobs=-1, eval_metric="logloss", tree_method="hist", **XGB_PARAMS)}
def fit(est, X, y):
    p = Pipeline([("prep", prep()), ("model", est)]); p.fit(X, y); return p

# ---------- customer data & the RR-17 split ----------
dsl = pd.read_csv(DSL, dtype={"project": str}); dsl["project"] = "dslib::" + dsl["project"]; dsl["task_type"] = "NA"
lab = dsl[dsl["is_late"].notna()].copy()
for l in ("is_late", "is_late_7", "grew"): lab[l] = lab[l].astype(int)
...
R = {"config": dict(seed=SEED, split="same 60/40 project split as RR-17", train_projects=int(tr["project"].nunique()), train_rows=int(len(tr)),
                    holdout_projects=int(te["project"].nunique()), holdout_rows=int(len(te)),
                    holdout_base_rates={l: round(float(te[l].mean()), 4) for l in ("is_late", "is_late_7", "grew")})}
te_b = te[te["sector_group"] == "buildings"]

# ---------- Part A: new-project hold-out ----------
A, fitted = {}, {}
for lbl in ("is_late", "is_late_7", "grew"):
    A[lbl] = {"reference_rel_position": (lambda a: round(max(a, 1 - a), 4))(auc(te[lbl], te["rel_position"].fillna(0))),
              "reference_duration": (lambda a: round(max(a, 1 - a), 4))(auc(te[lbl], te["planned_duration_days"].fillna(0))),
              }
    for name, est in models().items():
        m = fit(est, tr, tr[lbl]); p = m.predict_proba(te)[:, 1]; fitted[(name, lbl)] = m
        A[lbl][name] = dict(pooled=auc(te[lbl], p), per_project=per_project(te, lbl, p),
                            buildings=auc(te_b[lbl], p[(te["sector_group"] == "buildings").to_numpy()]),
                            by_group={g: auc(x[lbl], p[(te["sector_group"] == g).to_numpy()]) for g, x in te.groupby("sector_group")})
    print("A", lbl, {k: (v["pooled"], v["per_project"]["median"] if v.get("per_project") else None) for k, v in A[lbl].items() if isinstance(v, dict)})
...
def temporal_split(df):
    ...
    for _, g in df.groupby("project"):
        ...
        if len(a) >= MIN_PER_SIDE and len(b) >= MIN_PER_SIDE: a_.append(a); b_.append(b)
    return pd.concat(a_), pd.concat(b_)
B_tr, B_te = temporal_split(lab)
B = {"n_train": int(len(B_tr)), "n_test": int(len(B_te)), "projects": int(B_tr["project"].nunique()), "results": {}}
for lbl in ("is_late", "grew"):
    B["results"][lbl] = {name: auc(B_te[lbl], fit(est, B_tr, B_tr[lbl]).predict_proba(B_te)[:, 1]) for name, est in models().items()}
    B["results"][lbl]["reference_rel_position"] = (lambda a: round(max(a, 1 - a), 4))(auc(B_te[lbl], B_te["rel_position"].fillna(0)))
print("B", B["results"])
R["part_B_within_project_scenarioB"] = B

# ---------- Part C: learning curve by number of customer projects ----------
train_projects = sorted(tr["project"].unique()); C = {"n_train_projects_available": len(train_projects), "curve": {}}
for lbl in ("is_late", "grew"):
    C["curve"][lbl] = {}
    for n in (5, 10, 20, 40, len(train_projects)):
        vals, meds = [], []
        for d in range(10 if n < len(train_projects) else 1):
            r2 = np.random.default_rng(SEED + d); sub = tr[tr["project"].isin(r2.choice(train_projects, n, replace=False))]
            if sub[lbl].nunique() < 2: continue
            p = fit(RandomForestClassifier(class_weight="balanced", random_state=SEED, n_jobs=-1, **RF_PARAMS), sub, sub[lbl]).predict_proba(te)[:, 1]
            vals.append(roc_auc_score(te[lbl], p)); pp = per_project(te, lbl, p); meds.append(pp["median"] if pp else np.nan)
        v = np.array(vals)
        C["curve"][lbl][str(n)] = dict(draws=len(v), pooled_median=round(float(np.median(v)), 4), pooled_iqr=round(float(np.percentile(v, 75) - np.percentile(v, 25)), 4),
                                       pooled_min=round(float(v.min()), 4), pooled_max=round(float(v.max()), 4),
                                       per_project_median_of_medians=round(float(np.nanmedian(meds)), 4), rows_median=int(np.median([len(tr[tr["project"].isin(np.random.default_rng(SEED + d).choice(train_projects, n, replace=False))]) for d in range(len(v))])))
...
I = pd.read_csv(INST); I["task_type"] = "NA"
I_tr, I_te = I[~I["project"].isin(hold)].copy(), I[I["project"].isin(hold)].copy()
D = {"holdout_instances": int(len(I_te)), "holdout_snapshots": int(I_te.groupby(["project", "tp"]).ngroups), "holdout_projects": int(I_te["project"].nunique()), "results": {}}
for lbl in ("is_late", "grew"):
    m = fitted[("random_forest", lbl)]
    I_te["m"] = m.predict_proba(I_te)[:, 1]  # plan-time model trained on train projects only
    ...
    for a_, b_ in GroupKFold(n_splits=5).split(I_tr, I_tr[lbl], groups=I_tr["project"]):
        ...
        oof[b_] = fit(RandomForestClassifier(class_weight="balanced", random_state=SEED, n_jobs=-1, **RF_PARAMS), sub_tr, sub_tr[lbl]).predict_proba(I_tr.iloc[b_])[:, 1]
    I_tr["m"] = oof
    drift = I_te["median_start_slip_so_far"].fillna(0); share_late = I_te["share_finished_late_so_far"].fillna(0)
    res = {"plan_time_model_alone": auc(I_te[lbl], I_te["m"]), "drift_rule_alone": (lambda a: round(max(a, 1 - a), 4))(auc(I_te[lbl], drift)),
           "share_finished_late_alone": (lambda a: round(max(a, 1 - a), 4))(auc(I_te[lbl], share_late)),
           "plan_time_model_per_project": per_project(I_te, lbl, I_te["m"])}
    cols = ["m", "median_start_slip_so_far", "share_finished_late_so_far"]
    comb = Pipeline([("imp", SimpleImputer(strategy="median")), ("sc", StandardScaler()), ("lr", LogisticRegression(max_iter=2000, class_weight="balanced"))])
    comb.fit(I_tr[cols], I_tr[lbl]); pc = comb.predict_proba(I_te[cols])[:, 1]
    res["combination_model+drift_logistic"] = auc(I_te[lbl], pc); res["combination_per_project"] = per_project(I_te, lbl, pc)
    res["combination_coefficients"] = dict(zip(cols, [round(float(c), 3) for c in comb.named_steps["lr"].coef_[0]]))
    res["gain_over_drift_rule"] = round(res["combination_model+drift_logistic"] - res["drift_rule_alone"], 4)
    D["results"][lbl] = res; print("D", lbl, {k: v for k, v in res.items() if not isinstance(v, dict)})
...
for lbl in ("is_late", "grew"):
    rf = fit(RandomForestClassifier(class_weight="balanced", random_state=SEED, n_jobs=-1, **RF_PARAMS), F, F[lbl])
    cal = CalibratedClassifierCV(FrozenEstimator(rf), method="sigmoid"); cal.fit(Cc, Cc[lbl])
    pu, pc = rf.predict_proba(te)[:, 1], cal.predict_proba(te)[:, 1]
    E["holdout"][lbl] = dict(brier_dummy=dummy_brier(te[lbl]), brier_uncalibrated=brier(te[lbl], pu), brier_calibrated=brier(te[lbl], pc),
                             mean_p_calibrated=round(float(pc.mean()), 4), base_rate=round(float(te[lbl].mean()), 4), auc_calibrated=auc(te[lbl], pc))
print("E", E["holdout"])
R["part_E_customer_calibration"] = E

# ---------- Part F: reverse transfer to OUR corpus ----------
own = pd.read_csv(OWN); own = own[~own["project"].isin(EXCLUDE_OWN)]; own = own[own["is_late"].notna()].copy()
own["is_late"] = own["is_late"].astype(str).str.lower().isin(["true", "1", "1.0"]).astype(int)
p = fitted[("random_forest", "is_late")].predict_proba(own[NUM + CAT])[:, 1]
R["part_F_reverse_transfer_to_own"] = dict(n=int(len(own)), pooled_auc=auc(own["is_late"], p), per_project=per_project(own, "is_late", p),
...
json.dump(R, open(OUT / "rr18_customer_sim.json", "w"), indent=1, default=str); print("saved")
